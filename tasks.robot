...
Order robots from RobotSpareBin Industries Inc
    ...
    FOR    ${row}    IN    @{orders}
        ...
        Preview The Robot
        Submit The Order
        ${file_pdf}=    Store The Receipt As A PDF File    ${row}[Order number]
        ${scr}=    Take A Screenshot Of The Robot    ${row}[Order number]
        Embed The Robot Screenshot To The Receipt PDF File    ${scr}    ${file_pdf}
...
Fill The Form
    ...
    [Arguments]    ${this_row}
    ${head}=    Convert To Integer    ${this_row}[Head]
    ${body}=    Convert To Integer    ${this_row}[Body]
    ${legs}=    Convert To Integer    ${this_row}[Legs]
    ${address}=    Convert To String    ${this_row}[Address]
